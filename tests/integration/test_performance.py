...
class TestPerformance:
    ...
    def test_lambda_execution_time(self, lambda_client):
        ...
        try:
            ...
            assert execution_time < 600, f"Lambda execution took too long: {execution_time:.2f} seconds"
    ...
    def test_lambda_memory_usage(self, cloudwatch_client):
        ...
        try:
            ...
            if 'Datapoints' in response and len(response['Datapoints']) > 0:
                ...
                avg_memory = sum(dp['Average'] for dp in response['Datapoints']) / len(response['Datapoints'])
                ...
                print(f"✅ Lambda memory usage: Max {max_memory:.1f}%, Avg {avg_memory:.1f}%")
            else:
                print("⚠️ No memory metrics available (function may not have run recently)")
    ...
    def test_lambda_duration_metrics(self, cloudwatch_client):
        ...
        try:
            ...
            if 'Datapoints' in response and len(response['Datapoints']) > 0:
                ...
                assert avg_duration < 300000, f"Average duration too long: {avg_duration}ms"
                
                print(f"✅ Lambda duration: Max {max_duration/1000:.1f}s, Avg {avg_duration/1000:.1f}s")
            else:
                print("⚠️ No duration metrics available (function may not have run recently)")
    ...
    def test_glue_crawler_performance(self, glue_client):
        ...
        try:
            ...
            if current_state == 'READY':
                print("✅ Glue crawler is already in READY state (performance test passed)")
                return
            ...
            while time.time() - start_time < max_wait_time:
                ...
                if state == 'READY':
                    ...
                    assert execution_time < 600, f"Crawler took too long: {execution_time:.2f} seconds"
            ...
            for i, query in enumerate(queries):
                ...
                while time.time() - start_time < max_wait_time:
                    ...
                    if status == 'SUCCEEDED':
                        ...
                        assert execution_time < 120, f"Query {i+1} took too long: {execution_time:.2f} seconds"
                        
                        print(f"✅ Athena query {i+1} performance: {execution_time:.2f} seconds")
                        break
                    elif status == 'FAILED':
    ...
    def test_data_volume_performance(self, s3_client):
        ...
        try:
            ...
            if 'Contents' in response:
                ...
                # Check file sizes
                large_files = [obj for obj in objects if obj['Size'] > 10 * 1024 * 1024]  # > 10MB
                ...
                # Should not have empty files
                assert len(small_files) == 0, f"Found {len(small_files)} small/empty files"
                ...
                print(f"✅ Data volume performance: {len(objects)} files, {total_size/1024/1024:.1f}MB total")
            else:
                pytest.fail("No objects found in S3")
    ...
    def test_concurrent_execution(self, lambda_client):
        ...
        try:
            ...
            assert success_count >= 2, f"Only {success_count}/3 concurrent executions succeeded"
            ...
            for metric in metrics:
                ...
                if 'Datapoints' in response and len(response['Datapoints']) > 0:
                    if metric == 'Errors':
                        ...
                        assert error_count == 0, f"Lambda errors detected: {error_count}"
                    elif metric == 'Throttles':
                        throttle_count = sum(dp['Sum'] for dp in response['Datapoints'])
                        assert throttle_count == 0, f"Lambda throttles detected: {throttle_count}"
                    elif metric == 'Invocations':
                        invocation_count = sum(dp['Sum'] for dp in response['Datapoints'])
    ...
    def test_end_to_end_performance(self, lambda_client, s3_client, glue_client, athena_client):
